...
from isaaclab.utils.math import quat_apply_inverse, quat_error_magnitude, yaw_quat
...
def _get_command_vector(env: ManagerBasedRLEnv, command_name: str) -> torch.Tensor:
    command_term = env.command_manager.get_term(command_name)
    if hasattr(command_term, "velocity_command"):
        return command_term.velocity_command
    return env.command_manager.get_command(command_name)


def _fixed_motion_tilt_error(
    env: ManagerBasedRLEnv,
    motion_command_name: str,
) -> tuple[torch.Tensor, MotionCommand]:
    """Return yaw-invariant tilt error relative to the fixed motion frame."""
    motion_cmd: MotionCommand = env.command_manager.get_term(motion_command_name)
    gravity_w = motion_cmd.robot.data.GRAVITY_VEC_W
    target_gravity_b = quat_apply_inverse(motion_cmd.anchor_quat_w, gravity_w)
    current_gravity_b = quat_apply_inverse(motion_cmd.robot_anchor_quat_w, gravity_w)
    target_gravity_b = target_gravity_b / torch.linalg.norm(target_gravity_b, dim=1, keepdim=True).clamp_min(1.0e-8)
    current_gravity_b = current_gravity_b / torch.linalg.norm(current_gravity_b, dim=1, keepdim=True).clamp_min(1.0e-8)
    cosine = torch.sum(target_gravity_b * current_gravity_b, dim=1).clamp(-1.0, 1.0)
    return torch.acos(cosine), motion_cmd


def _ready_segment_heading_yaw_error(
    env: ManagerBasedRLEnv,
    command_name: str,
    yaw_command_deadzone: float,
    motion_command_name: str,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Return heading error anchored at the end of the latest yaw-command segment."""
    motion_cmd: MotionCommand = env.command_manager.get_term(motion_command_name)
    command = env.command_manager.get_command(command_name)
    yaw_inactive = torch.abs(command[:, 2]) < yaw_command_deadzone
    current_heading = yaw_quat(motion_cmd.robot_anchor_quat_w)

    anchor_attr = f"_ready_heading_anchor_{motion_command_name}_{command_name}"
    heading_anchor = getattr(env, anchor_attr, None)
    if heading_anchor is None or heading_anchor.shape != current_heading.shape:
        heading_anchor = current_heading.detach().clone()

    # Before ready and throughout a commanded turn, follow the current heading.
    # Once yaw becomes inactive, hold the latest heading as the new segment target.
    refresh_anchor = (~motion_cmd.ready) | (~yaw_inactive)
    heading_anchor = torch.where(refresh_anchor.unsqueeze(-1), current_heading.detach(), heading_anchor)
    setattr(env, anchor_attr, heading_anchor.detach())

    yaw_error = quat_error_magnitude(heading_anchor, current_heading)
    return yaw_error, motion_cmd.ready & yaw_inactive
...
def y_command_motion_relative_body_position_error_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    body_names: list[str],
    ...
    """Track selected reference body positions under lateral command, optionally one direction."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    command_vector = _get_command_vector(env, command_name)
    if direction is None:
        gate = torch.abs(command_vector[:, 1]) > command_threshold
    else:
        direction_sign = 1.0 if direction >= 0.0 else -1.0
        gate = command_vector[:, 1] * direction_sign > command_threshold
...
def y_command_motion_body_z_position_error_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    body_names: list[str],
    ...
    """Track selected reference body heights under lateral command."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    command_vector = _get_command_vector(env, command_name)
    if direction is None:
        gate = torch.abs(command_vector[:, 1]) > command_threshold
    ...
    body_indexes = _get_body_indexes(command, body_names)
    reference_z = command.body_pos_w[:, body_indexes, 2]
    robot_z = command.robot_body_pos_w[:, body_indexes, 2]
    error = torch.square(reference_z - robot_z)
    reward = torch.exp(-error.mean(-1) / std**2)
    return reward * gate.to(reward.dtype)


def y_command_motion_yaw_pair_spread_error_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    body_names: list[str],
    axis: int = 1,
    direction: float | None = None,
    command_threshold: float = 0.03,
    apply_at_zero: bool = False,
    deadband: float = 0.025,
    scale: float = 0.10,
    max_penalty: float | None = 1.0,
) -> torch.Tensor:
    """Penalize a two-body yaw-frame spread changing away from the reference motion."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    command_vector = _get_command_vector(env, command_name)
    if apply_at_zero:
        gate = torch.ones(command_vector.shape[0], dtype=torch.bool, device=command_vector.device)
    ...
    if len(body_indexes) != 2:
        raise ValueError("y_command_motion_yaw_pair_spread_error_l2_penalty expects exactly two bodies.")

    robot_delta_w = command.robot_body_pos_w[:, body_indexes[0]] - command.robot_body_pos_w[:, body_indexes[1]]
    reference_delta_w = command.body_pos_relative_w[:, body_indexes[0]] - command.body_pos_relative_w[:, body_indexes[1]]

    robot_delta_yaw = quat_apply_inverse(yaw_quat(command.robot_anchor_quat_w), robot_delta_w)
    reference_delta_yaw = quat_apply_inverse(yaw_quat(command.anchor_quat_w), reference_delta_w)

    spread_error = torch.abs(robot_delta_yaw[:, axis]) - torch.abs(reference_delta_yaw[:, axis])
    penalty = torch.square(torch.clamp(torch.abs(spread_error) - deadband, min=0.0) / max(scale, 1.0e-6))
...
def reference_anchor_orientation_error_l2(
    env: ManagerBasedRLEnv,
    command_name: str,
) -> torch.Tensor:
    """Penalize base-orientation deviation from the current reference frame."""
    command = env.command_manager.get_term(command_name)
    error = quat_error_magnitude(command.anchor_quat_w, command.robot_anchor_quat_w)
    return torch.square(error)


def moving_reference_anchor_orientation_error_l2(
    env: ManagerBasedRLEnv,
    command_name: str,
    command_threshold: float = 0.03,
) -> torch.Tensor:
    """Penalize reference-orientation error only while a motion command is active."""
    penalty = reference_anchor_orientation_error_l2(env, command_name=command_name)
    command = _get_command_vector(env, command_name)
    moving = torch.linalg.norm(command, dim=1) > command_threshold
    return penalty * moving.to(penalty.dtype)


def track_lateral_reference_velocity_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Track the selected reference's y velocity in the robot yaw frame."""
    command = env.command_manager.get_term(command_name)
    ...
    error = torch.square(command.velocity_command[:, 1] - velocity_y)
    return torch.exp(-error / std**2)


def track_reference_velocity_axis_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    axis: int,
    std: float,
    min_abs_command: float = 0.0,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Track one commanded velocity axis in the yaw-projected body frame."""
    command = _get_command_vector(env, command_name)
    command_axis = command[:, axis]
    gate = torch.abs(command_axis) >= min_abs_command

    asset = env.scene[asset_cfg.name]
    yaw_only_quat = yaw_quat(asset.data.root_quat_w)
    velocity_axis = quat_apply_inverse(yaw_only_quat, asset.data.root_lin_vel_w)[:, axis]
    error = torch.square(command_axis - velocity_axis)
    reward = torch.exp(-error / std**2)
    return reward * gate.to(reward.dtype)


def track_lateral_reference_velocity_band_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    ...
    """Track lateral velocity only inside a commanded speed band."""
    command = env.command_manager.get_term(command_name)
    command_y = command.velocity_command[:, 1]
    command_abs = torch.abs(command_y)
    gate = (command_abs >= min_abs_command) & (command_abs <= max_abs_command)

    asset = env.scene[asset_cfg.name]
    yaw_only_quat = yaw_quat(asset.data.root_quat_w)
    velocity_y = quat_apply_inverse(yaw_only_quat, asset.data.root_lin_vel_w)[:, 1]
    error = torch.square(command_y - velocity_y)
    reward = torch.exp(-error / std**2)
    return reward * gate.to(reward.dtype)


def track_lateral_reference_velocity_direction_band_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    min_abs_command: float,
    ...
    """Track lateral velocity inside one signed command-speed band."""
    command = _get_command_vector(env, command_name)
    command_y = command[:, 1]
    direction_sign = 1.0 if direction >= 0.0 else -1.0
    signed_speed = command_y * direction_sign
    gate = (signed_speed >= min_abs_command) & (signed_speed <= max_abs_command)
...
def stage_gated_reference_anchor_orientation_error_l2(
    env: ManagerBasedRLEnv,
    command_name: str,
    stage_command_name: str = "motion",
    enabled_stage: int = 2,
) -> torch.Tensor:
    base_penalty = reference_anchor_orientation_error_l2(env, command_name=command_name)
    motion_cmd: MotionCommand = env.command_manager.get_term(stage_command_name)
    gate = (motion_cmd.stage >= enabled_stage).to(base_penalty.dtype)
    return base_penalty * gate


def stage_gated_track_lateral_reference_velocity_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    stage_command_name: str = "motion",
    ...
    base_reward = track_lateral_reference_velocity_exp(env, command_name=command_name, std=std, asset_cfg=asset_cfg)
    motion_cmd: MotionCommand = env.command_manager.get_term(stage_command_name)
...
def y_direction_feet_contact_reward(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    direction: float,
    ...
    """Reward selected feet carrying current vertical load for one lateral direction."""
    command = _get_command_vector(env, command_name)
    direction_sign = 1.0 if direction >= 0.0 else -1.0
    y_direction_active = command[:, 1] * direction_sign > command_threshold

    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    force_z = torch.abs(contact_sensor.data.net_forces_w[:, sensor_cfg.body_ids, 2])
    is_contact = force_z > force_threshold
    reward = is_contact.to(torch.float32).mean(dim=1)
    return reward * y_direction_active.to(reward.dtype)


def y_direction_feet_force_balance_l1_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    direction: float,
    ...
    """Penalize uneven selected-foot vertical loading for one lateral direction."""
    command = _get_command_vector(env, command_name)
    direction_sign = 1.0 if direction >= 0.0 else -1.0
    y_direction_active = command[:, 1] * direction_sign > command_threshold
    ...
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    force_z = torch.abs(contact_sensor.data.net_forces_w[:, sensor_cfg.body_ids, 2])
    if force_z.shape[1] != 2:
        raise ValueError("y_direction_feet_force_balance_l1_penalty expects exactly two feet.")

    both_loaded = torch.minimum(force_z[:, 0], force_z[:, 1]) > force_threshold
    imbalance = torch.abs(force_z[:, 0] - force_z[:, 1]) / max(force_scale, 1.0e-6)
    penalty = torch.where(both_loaded, imbalance, torch.ones_like(imbalance))
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * y_direction_active.to(penalty.dtype)


def y_command_feet_contact_reward(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    command_threshold: float = 0.03,
    force_threshold: float = 12.0,
) -> torch.Tensor:
    """Reward selected feet carrying current vertical load under any lateral command."""
    command = _get_command_vector(env, command_name)
    ...
    is_contact = force_z > force_threshold
    reward = is_contact.to(torch.float32).mean(dim=1)
    return reward * y_active.to(reward.dtype)


def y_command_reference_low_feet_contact_reward(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    body_names: list[str],
    command_threshold: float = 0.03,
    force_threshold: float = 8.0,
    support_height_margin: float = 0.018,
) -> torch.Tensor:
    """Reward contact only for feet whose reference height is in the support band."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    command_vector = _get_command_vector(env, command_name)
    y_active = torch.abs(command_vector[:, 1]) > command_threshold

    body_indexes = _get_body_indexes(command, body_names)
    if len(body_indexes) != len(sensor_cfg.body_ids):
        raise ValueError("body_names and sensor_cfg body ids must describe the same feet.")

    reference_z = command.body_pos_w[:, body_indexes, 2]
    support_z = reference_z.min(dim=1, keepdim=True).values
    support_mask = reference_z <= support_z + support_height_margin

    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    force_z = torch.abs(contact_sensor.data.net_forces_w[:, sensor_cfg.body_ids, 2])
    is_contact = force_z > force_threshold

    support_count = support_mask.to(torch.float32).sum(dim=1).clamp_min(1.0)
    reward = (is_contact & support_mask).to(torch.float32).sum(dim=1) / support_count
    return reward * y_active.to(reward.dtype)


def zero_command_feet_contact_reward(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    ...
    """Reward selected feet carrying load when the commanded velocity is near zero."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband

    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    force_z = torch.abs(contact_sensor.data.net_forces_w[:, sensor_cfg.body_ids, 2])
    is_contact = force_z > force_threshold
    reward = is_contact.to(torch.float32).mean(dim=1)
    return reward * cmd_small.to(reward.dtype)


def y_command_feet_force_balance_l1_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    command_threshold: float = 0.03,
    ...
    """Penalize uneven selected-foot vertical loading under any lateral command."""
    command = _get_command_vector(env, command_name)
    y_active = torch.abs(command[:, 1]) > command_threshold
    ...
    force_z = torch.abs(contact_sensor.data.net_forces_w[:, sensor_cfg.body_ids, 2])
    if force_z.shape[1] != 2:
        raise ValueError("y_command_feet_force_balance_l1_penalty expects exactly two feet.")
    ...
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * y_active.to(penalty.dtype)


def stage_gated_front_feet_lateral_separation_penalty(
    env: ManagerBasedRLEnv,
    command_name: str = "base_velocity",
    stage_command_name: str = "motion",
    enabled_stage: int = 2,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    left_body_name: str = "FL_foot_link",
    right_body_name: str = "FR_foot_link",
    min_separation: float = 0.12,
    command_axis: int = 1,
    command_deadband: float = 0.02,
) -> torch.Tensor:
    """Penalize front feet getting too close during commanded stage2 lateral motion."""
    motion_cmd: MotionCommand = env.command_manager.get_term(stage_command_name)
    stage_gate = motion_cmd.stage >= enabled_stage

    command = _get_command_vector(env, command_name)
    command_gate = torch.abs(command[:, command_axis]) > command_deadband

    asset = env.scene[asset_cfg.name]
    left_ids, _ = asset.find_bodies([left_body_name], preserve_order=True)
    right_ids, _ = asset.find_bodies([right_body_name], preserve_order=True)
    if len(left_ids) != 1 or len(right_ids) != 1:
        raise ValueError(f"Expected one body for {left_body_name} and {right_body_name}.")

    left_pos_w = asset.data.body_pos_w[:, left_ids[0], :]
    right_pos_w = asset.data.body_pos_w[:, right_ids[0], :]
    delta_w = left_pos_w - right_pos_w
    # Measure left-right separation in the robot yaw frame, not the world y axis.
    delta_yaw = quat_apply_inverse(yaw_quat(asset.data.root_quat_w), delta_w)
    separation = torch.abs(delta_yaw[:, 1])
    penalty = torch.square(torch.clamp(min_separation - separation, min=0.0))
    return penalty * (stage_gate & command_gate).to(penalty.dtype)
...
def y_command_diagonal_leg_motion_reward(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg,
    diag_a_joint_names: list[str],
    diag_b_joint_names: list[str],
    y_cmd_threshold: float = 0.03,
    max_value: float = 2.0,
) -> torch.Tensor:
    """Reward both diagonal leg pairs participating under a lateral y command."""
    cmd = _get_command_vector(env, command_name)
    y_gate = (torch.abs(cmd[:, 1]) > y_cmd_threshold).float()

    asset = env.scene[asset_cfg.name]
    diag_a_ids, _ = asset.find_joints(diag_a_joint_names)
    diag_b_ids, _ = asset.find_joints(diag_b_joint_names)

    motion_a = torch.mean(torch.abs(asset.data.joint_vel[:, diag_a_ids]), dim=1)
    motion_b = torch.mean(torch.abs(asset.data.joint_vel[:, diag_b_ids]), dim=1)
    motion_a = torch.clamp(motion_a, max=max_value) / max_value
    motion_b = torch.clamp(motion_b, max=max_value) / max_value

    return torch.minimum(motion_a, motion_b) * y_gate


def y_command_diagonal_motion_balance_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg,
    diag_a_joint_names: list[str],
    ...
    """Penalize one diagonal leg pair dominating during lateral y command."""
    cmd = _get_command_vector(env, command_name)
    y_gate = (torch.abs(cmd[:, 1]) > y_cmd_threshold).float()
    ...
    return torch.abs(motion_a - motion_b) * y_gate


def y_command_rear_sync_without_front_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg,
    rear_left_joint_names: list[str],
    rear_right_joint_names: list[str],
    front_left_joint_names: list[str],
    front_right_joint_names: list[str],
    y_cmd_threshold: float = 0.03,
    min_rear_sync: float = 0.18,
    front_relief: float = 0.45,
    max_value: float = 2.0,
    max_penalty: float | None = 1.0,
) -> torch.Tensor:
    """Penalize rear-leg synchronous scraping during lateral commands.

    The diagonal participation reward only checks that both diagonal groups
    have motion. A bad local optimum can satisfy it by moving both rear legs
    together while the front support mostly drags. This term targets that
    specific failure mode without forcing the front wheels to lift from the
    trunk: rear-left and rear-right may move, but if they move together their
    motion should be accompanied by comparable front-leg participation.
    """
    command = _get_command_vector(env, command_name)
    y_gate = (torch.abs(command[:, 1]) > y_cmd_threshold).float()

    asset = env.scene[asset_cfg.name]
    rear_left_ids, _ = asset.find_joints(rear_left_joint_names)
    rear_right_ids, _ = asset.find_joints(rear_right_joint_names)
    front_left_ids, _ = asset.find_joints(front_left_joint_names)
    front_right_ids, _ = asset.find_joints(front_right_joint_names)

    rear_left_motion = torch.mean(torch.abs(asset.data.joint_vel[:, rear_left_ids]), dim=1)
    rear_right_motion = torch.mean(torch.abs(asset.data.joint_vel[:, rear_right_ids]), dim=1)
    front_left_motion = torch.mean(torch.abs(asset.data.joint_vel[:, front_left_ids]), dim=1)
    front_right_motion = torch.mean(torch.abs(asset.data.joint_vel[:, front_right_ids]), dim=1)

    rear_sync = torch.minimum(rear_left_motion, rear_right_motion)
    front_participation = torch.maximum(front_left_motion, front_right_motion)
    excess = torch.clamp(rear_sync - front_relief * front_participation - min_rear_sync, min=0.0)
    penalty = torch.square(excess / max(max_value, 1.0e-6))
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * y_gate


def box_local_axis_position_deviation_l2_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    box_name: str = "box",
    axis: int = 0,
    ...
    """Penalize drift from the initial local position relative to a box-like support."""
    asset = env.scene[asset_cfg.name]
    ...
    box_yaw = yaw_quat(box.data.root_quat_w).unsqueeze(1).expand(-1, num_bodies, -1)
    rel_local = quat_apply_inverse(box_yaw.reshape(-1, 4), rel_w.reshape(-1, 3)).reshape(num_envs, num_bodies, 3)
    current = rel_local[..., axis]

    cache_attr = f"_box_local_axis_target_{box_name}_{asset_cfg.name}_{axis}_{cache_key}"
    target = getattr(env, cache_attr, None)
    if target is None or target.shape != current.shape:
        target = current.detach().clone()
        setattr(env, cache_attr, target)

    excess = torch.clamp(torch.abs(current - target) - deadband, min=0.0)
...
def box_local_axis_position_deviation_reset_l2_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    command_name: str = "motion",
    box_name: str = "box",
    ...
    """Penalize drift from the per-episode reset local position relative to a box."""
    asset = env.scene[asset_cfg.name]
    box = env.scene[box_name]
    command: MotionCommand = env.command_manager.get_term(command_name)
    ...
    cache_attr = f"_box_local_axis_reset_target_{box_name}_{asset_cfg.name}_{axis}_{cache_key}"
    target = getattr(env, cache_attr, None)
    if target is None or target.shape != current.shape:
        target = current.detach().clone()
    ...
    excess = torch.clamp(torch.abs(current - target) - deadband, min=0.0)
    penalty = torch.mean(torch.square(excess / max(position_scale, 1.0e-6)), dim=1)
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty


def zero_command_box_local_axis_position_deviation_reset_l2_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    command_name: str = "motion",
    box_name: str = "box",
    axis: int = 0,
    command_deadband: float = 0.030,
    ...
    """Penalize reset-relative local-axis body drift only when command is zero."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband
    penalty = box_local_axis_position_deviation_reset_l2_penalty(
        env,
        asset_cfg=asset_cfg,
    ...
    return penalty * cmd_small.to(penalty.dtype)


def zero_command_box_local_axis_position_hold_latched_l2_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    command_name: str = "motion",
    box_name: str = "box",
    ...
    """Hold the local-axis position captured when a moving command becomes zero."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband
    asset = env.scene[asset_cfg.name]
    box = env.scene[box_name]
    ...
    num_envs, num_bodies, _ = rel_w.shape
    box_yaw = yaw_quat(box.data.root_quat_w).unsqueeze(1).expand(-1, num_bodies, -1)
    rel_local = quat_apply_inverse(box_yaw.reshape(-1, 4), rel_w.reshape(-1, 3)).reshape(
        num_envs, num_bodies, 3
    )
    current = rel_local[..., axis]

    target_attr = f"_zero_latched_axis_target_{box_name}_{asset_cfg.name}_{axis}_{cache_key}"
    state_attr = f"_zero_latched_axis_state_{box_name}_{asset_cfg.name}_{axis}_{cache_key}"
    target = getattr(env, target_attr, None)
    previous_small = getattr(env, state_attr, None)
    if target is None or target.shape != current.shape:
        target = current.detach().clone()
    if previous_small is None or previous_small.shape != cmd_small.shape:
        previous_small = torch.zeros_like(cmd_small)

    episode_start = env.episode_length_buf <= 1
    entered_hold = cmd_small & ~previous_small
    refresh = episode_start | entered_hold
    if torch.any(refresh):
        target = target.clone()
        target[refresh] = current.detach()[refresh]
    setattr(env, target_attr, target.detach())
    setattr(env, state_attr, cmd_small.detach().clone())
...
def zero_command_joint_position_hold_latched_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg,
    command_deadband: float = 0.025,
    ...
    max_penalty: float | None = 2.0,
    cache_key: str = "default",
) -> torch.Tensor:
    """Hold selected joint angles captured at the moving-to-zero transition."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband
    asset = env.scene[asset_cfg.name]
    joint_ids = _get_joint_ids(asset_cfg)
    ...
    target_attr = f"_zero_latched_joint_target_{asset_cfg.name}_{cache_key}"
    state_attr = f"_zero_latched_joint_state_{asset_cfg.name}_{cache_key}"
    target = getattr(env, target_attr, None)
    previous_small = getattr(env, state_attr, None)
    if target is None or target.shape != current.shape:
        target = current.detach().clone()
...
def box_local_axis_pair_difference_l2_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    box_name: str = "box",
    axis: int = 0,
    ...
    """Penalize two selected bodies separating too much along a box-local axis."""
    asset = env.scene[asset_cfg.name]
    box = env.scene[box_name]

    body_pos_w = asset.data.body_pos_w[:, asset_cfg.body_ids, :]
    if body_pos_w.shape[1] != 2:
        raise ValueError("box_local_axis_pair_difference_l2_penalty expects exactly two bodies.")
    ...
    pair_diff = torch.abs(rel_local[:, 0, axis] - rel_local[:, 1, axis])
    penalty = torch.square(torch.clamp(pair_diff - deadband, min=0.0))
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty


def zero_command_box_local_axis_pair_difference_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg,
    box_name: str = "box",
    ...
    """Penalize two selected bodies separating along a box-local axis at zero command."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband
    ...
    body_pos_w = asset.data.body_pos_w[:, asset_cfg.body_ids, :]
    if body_pos_w.shape[1] != 2:
        raise ValueError("zero_command_box_local_axis_pair_difference_l2_penalty expects exactly two bodies.")
...
def motion_yaw_axis_pair_difference_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg,
    axis: int = 0,
    ...
    """Penalize two selected bodies separating along the motion anchor yaw frame."""
    command_vector = _get_command_vector(env, command_name)
    if command_deadband is None:
        gate = torch.ones(command_vector.shape[0], device=command_vector.device, dtype=torch.bool)
    else:
        gate = torch.linalg.norm(command_vector, dim=1) < command_deadband

    motion_cmd: MotionCommand = env.command_manager.get_term(command_name)
    if frame == "robot":
        frame_quat_w = yaw_quat(motion_cmd.robot_anchor_quat_w)
        frame_pos_w = motion_cmd.robot_anchor_pos_w
    elif frame == "reference":
        frame_quat_w = yaw_quat(motion_cmd.anchor_quat_w)
        frame_pos_w = motion_cmd.anchor_pos_w
    else:
        raise ValueError("motion_yaw_axis_pair_difference_l2_penalty frame must be 'robot' or 'reference'.")

    asset = env.scene[asset_cfg.name]
    body_pos_w = asset.data.body_pos_w[:, asset_cfg.body_ids, :]
    if body_pos_w.shape[1] != 2:
        raise ValueError("motion_yaw_axis_pair_difference_l2_penalty expects exactly two bodies.")

    rel_w = body_pos_w - frame_pos_w[:, None, :]
    num_envs, num_bodies, _ = rel_w.shape
    frame_quat_w = frame_quat_w.unsqueeze(1).expand(-1, num_bodies, -1)
    rel_local = quat_apply_inverse(frame_quat_w.reshape(-1, 4), rel_w.reshape(-1, 3)).reshape(num_envs, num_bodies, 3)
...
def motion_yaw_pair_target_distance_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg,
    axis: int,
    ...
    """Track an absolute two-body spacing in a motion-anchor yaw frame.

    Unlike :func:`motion_yaw_axis_pair_difference_l2_penalty`, this term is
    symmetric: it penalizes a pair that is either too narrow or too wide.  The
    explicit scale also keeps centimeter-level errors meaningful to PPO.
    """
    command_vector = _get_command_vector(env, command_name)
    command_norm = torch.linalg.norm(command_vector, dim=1)
    if command_mode == "moving":
        gate = command_norm > command_threshold
    elif command_mode == "zero":
        gate = command_norm < command_threshold
    elif command_mode == "all":
        gate = torch.ones(command_norm.shape, device=command_norm.device, dtype=torch.bool)
    else:
        raise ValueError("motion_yaw_pair_target_distance_l2_penalty command_mode must be 'moving', 'zero', or 'all'.")
    ...
    if frame == "robot":
        ...
        raise ValueError("motion_yaw_pair_target_distance_l2_penalty frame must be 'robot' or 'reference'.")

    asset = env.scene[asset_cfg.name]
    body_pos_w = asset.data.body_pos_w[:, asset_cfg.body_ids, :]
    if body_pos_w.shape[1] != 2:
        raise ValueError("motion_yaw_pair_target_distance_l2_penalty expects exactly two bodies.")
    ...
    pair_distance = torch.abs(rel_local[:, 0, axis] - rel_local[:, 1, axis])
    distance_error = torch.abs(pair_distance - target_distance)
    penalty = torch.square(torch.clamp(distance_error - deadband, min=0.0) / max(scale, 1.0e-6))
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * gate.to(penalty.dtype)


def y_direction_motion_yaw_axis_pair_difference_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg,
    direction: float,
    ...
    """Penalize pair separation in a yaw frame only for one lateral command direction."""
    command_vector = _get_command_vector(env, command_name)
    direction_sign = 1.0 if direction >= 0.0 else -1.0
    gate = command_vector[:, 1] * direction_sign > command_threshold
    ...
    if frame == "robot":
        ...
        raise ValueError("y_direction_motion_yaw_axis_pair_difference_l2_penalty frame must be 'robot' or 'reference'.")

    asset = env.scene[asset_cfg.name]
    body_pos_w = asset.data.body_pos_w[:, asset_cfg.body_ids, :]
    if body_pos_w.shape[1] != 2:
        raise ValueError("y_direction_motion_yaw_axis_pair_difference_l2_penalty expects exactly two bodies.")
...
def full_box_local_axis_pair_difference_l2_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    box_name: str = "box",
    axis: int = 0,
    ...
    """Penalize two selected bodies separating along the full box-local axis."""
    asset = env.scene[asset_cfg.name]
    box = env.scene[box_name]

    body_pos_w = asset.data.body_pos_w[:, asset_cfg.body_ids, :]
    if body_pos_w.shape[1] != 2:
        raise ValueError("full_box_local_axis_pair_difference_l2_penalty expects exactly two bodies.")

    rel_w = body_pos_w - box.data.root_pos_w[:, None, :]
    num_envs, num_bodies, _ = rel_w.shape
    box_quat_w = box.data.root_quat_w.unsqueeze(1).expand(-1, num_bodies, -1)
    rel_local = quat_apply_inverse(box_quat_w.reshape(-1, 4), rel_w.reshape(-1, 3)).reshape(num_envs, num_bodies, 3)
...
def y_direction_full_box_local_axis_drop_margin_reset_max_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    direction: float,
    command_name: str = "motion",
    ...
    axis: int = 2,
    command_threshold: float = 0.03,
    drop_margin: float = 0.010,
    reset_steps: int = 2,
    max_penalty: float | None = 1.0,
    cache_key: str = "default",
) -> torch.Tensor:
    """Penalize selected bodies dropping in full box-local frame for one lateral command direction."""
    command_vector = _get_command_vector(env, command_name)
    direction_sign = 1.0 if direction >= 0.0 else -1.0
    gate = command_vector[:, 1] * direction_sign > command_threshold
    ...
    cache_attr = f"_y_dir_full_box_axis_drop_max_target_{box_name}_{asset_cfg.name}_{axis}_{cache_key}"
    target = getattr(env, cache_attr, None)
    if target is None or target.shape != current.shape:
        target = current.detach().clone()
    ...
    drop_excess = torch.clamp((target - current) - drop_margin, min=0.0)
    penalty = torch.max(torch.square(drop_excess), dim=1).values
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * gate.to(penalty.dtype)


def full_box_local_axis_drop_margin_reset_max_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    command_name: str = "motion",
    box_name: str = "box",
    axis: int = 2,
    drop_margin: float = 0.012,
    reset_steps: int = 2,
    max_penalty: float | None = 1.0,
    cache_key: str = "default",
) -> torch.Tensor:
    """Penalize the worst selected body dropping in the full box-local frame."""
    asset = env.scene[asset_cfg.name]
    box = env.scene[box_name]
    command: MotionCommand = env.command_manager.get_term(command_name)
    ...
    cache_attr = f"_full_box_local_axis_drop_max_target_{box_name}_{asset_cfg.name}_{axis}_{cache_key}"
    target = getattr(env, cache_attr, None)
    if target is None or target.shape != current.shape:
        target = current.detach().clone()
...
def box_local_axis_four_body_skew_l2_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    box_name: str = "box",
    axis: int = 0,
    ...
    """Penalize wheel x skew between left and right side body pairs.

    The expected body order is FL, FR, RL, RR. The penalty compares the local-x
    front-to-rear offset on the left side with the same offset on the right side.
    This keeps the support polygon from twisting into a one-side-forward shape.
    """
    asset = env.scene[asset_cfg.name]
    box = env.scene[box_name]
    ...
    if body_pos_w.shape[1] != 4:
        raise ValueError("box_local_axis_four_body_skew_l2_penalty expects exactly four bodies: FL, FR, RL, RR.")
    ...
    front_rear_left = rel_local[:, 0, axis] - rel_local[:, 2, axis]
    front_rear_right = rel_local[:, 1, axis] - rel_local[:, 3, axis]
    skew = torch.abs(front_rear_left - front_rear_right)
    penalty = torch.square(torch.clamp(skew - deadband, min=0.0))
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty


def box_local_axis_backward_margin_reset_l2_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    command_name: str = "motion",
    box_name: str = "box",
    axis: int = 0,
    backward_margin: float = 0.025,
    reset_steps: int = 2,
    max_penalty: float | None = 1.0,
    cache_key: str = "default",
) -> torch.Tensor:
    """Penalize selected bodies moving backward from their reset local-axis position.

    Unlike symmetric x drift, this only penalizes motion toward the ramp/drop
    side. The sign convention assumes decreasing box-local axis value is the
    unsafe backward direction.
    """
    asset = env.scene[asset_cfg.name]
    box = env.scene[box_name]
    command: MotionCommand = env.command_manager.get_term(command_name)
    ...
    cache_attr = f"_box_local_axis_backward_target_{box_name}_{asset_cfg.name}_{axis}_{cache_key}"
    target = getattr(env, cache_attr, None)
    if target is None or target.shape != current.shape:
        target = current.detach().clone()
    ...
    backward_excess = torch.clamp((target - current) - backward_margin, min=0.0)
    penalty = torch.mean(torch.square(backward_excess), dim=1)
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty


def box_local_axis_backward_margin_reset_max_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    command_name: str = "motion",
    box_name: str = "box",
    axis: int = 0,
    backward_margin: float = 0.020,
    ...
    """Penalize the worst selected body moving toward the unsafe side from reset.

    This is one-sided and uses max over bodies instead of mean, so one front
    wheel drifting toward the ramp/drop edge cannot be hidden by the other wheel.
    """
    asset = env.scene[asset_cfg.name]
    box = env.scene[box_name]
    command: MotionCommand = env.command_manager.get_term(command_name)
    ...
    cache_attr = f"_box_local_axis_backward_max_target_{box_name}_{asset_cfg.name}_{axis}_{cache_key}"
    target = getattr(env, cache_attr, None)
    if target is None or target.shape != current.shape:
        target = current.detach().clone()
    ...
    unsafe_delta = direction * (current - target)
    excess = torch.clamp(unsafe_delta - backward_margin, min=0.0)
    penalty = torch.max(torch.square(excess / max(position_scale, 1.0e-6)), dim=1).values
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty


def box_local_axis_drop_margin_reset_max_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    command_name: str = "motion",
    box_name: str = "box",
    ...
    """Penalize the worst selected body dropping below its reset local-axis value."""
    asset = env.scene[asset_cfg.name]
    box = env.scene[box_name]
    command: MotionCommand = env.command_manager.get_term(command_name)
    ...
    cache_attr = f"_box_local_axis_drop_max_target_{box_name}_{asset_cfg.name}_{axis}_{cache_key}"
    target = getattr(env, cache_attr, None)
    if target is None or target.shape != current.shape:
        target = current.detach().clone()
    ...
    drop_excess = torch.clamp((target - current) - drop_margin, min=0.0)
    penalty = torch.max(torch.square(drop_excess / max(position_scale, 1.0e-6)), dim=1).values
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty


def zero_command_box_local_axis_backward_margin_reset_max_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    command_name: str = "motion",
    command_deadband: float = 0.060,
    box_name: str = "box",
    axis: int = 0,
    backward_margin: float = 0.020,
    unsafe_direction: float = -1.0,
    ...
    position_scale: float = 1.0,
    max_penalty: float | None = 1.0,
    cache_key: str = "zero_cmd",
) -> torch.Tensor:
    """Apply the reset-relative backward safety margin only when command is near zero."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband
    penalty = box_local_axis_backward_margin_reset_max_penalty(
        env=env,
        asset_cfg=asset_cfg,
        command_name=command_name,
        box_name=box_name,
...
def zero_command_box_local_axis_drop_margin_reset_max_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    command_name: str = "motion",
    box_name: str = "box",
    axis: int = 2,
    command_deadband: float = 0.030,
    drop_margin: float = 0.020,
    reset_steps: int = 2,
    max_penalty: float | None = 1.0,
    cache_key: str = "default",
) -> torch.Tensor:
    """Penalize reset-relative local-axis body drop only when command is zero."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband
    penalty = box_local_axis_drop_margin_reset_max_penalty(
        env,
        asset_cfg=asset_cfg,
        command_name=command_name,
        box_name=box_name,
...
def box_local_axis_rise_margin_reset_max_penalty(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
    command_name: str = "motion",
    box_name: str = "box",
    axis: int = 2,
    rise_margin: float = 0.025,
    reset_steps: int = 2,
    max_penalty: float | None = 1.0,
    cache_key: str = "default",
) -> torch.Tensor:
    """Penalize the worst selected body rising above its reset local-axis value."""
    asset = env.scene[asset_cfg.name]
    box = env.scene[box_name]
    command: MotionCommand = env.command_manager.get_term(command_name)
    ...
    cache_attr = f"_box_local_axis_rise_max_target_{box_name}_{asset_cfg.name}_{axis}_{cache_key}"
    target = getattr(env, cache_attr, None)
    if target is None or target.shape != current.shape:
        target = current.detach().clone()
    ...
    rise_excess = torch.clamp((current - target) - rise_margin, min=0.0)
    penalty = torch.max(torch.square(rise_excess), dim=1).values
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty


def zero_command_wheel_motion_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg,
    command_deadband: float = 0.03,
    wheel_velocity_deadband: float = 0.25,
    max_penalty: float | None = 4.0,
) -> torch.Tensor:
    """Penalize wheel rolling when the commanded velocity is near zero."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband
    ...
    excess = torch.clamp(torch.abs(wheel_vel) - wheel_velocity_deadband, min=0.0)
    penalty = torch.mean(torch.square(excess), dim=1)
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * cmd_small.to(penalty.dtype)
...
def zero_command_base_lateral_velocity_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    command_deadband: float = 0.025,
    velocity_deadband: float = 0.015,
    velocity_scale: float = 0.08,
    max_penalty: float | None = 2.0,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Penalize base lateral drift when the commanded velocity is zero."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband

    asset = env.scene[asset_cfg.name]
    yaw_only_quat = yaw_quat(asset.data.root_quat_w)
    lin_vel_yaw = quat_apply_inverse(yaw_only_quat, asset.data.root_lin_vel_w)
    excess = torch.clamp(torch.abs(lin_vel_yaw[:, 1]) - velocity_deadband, min=0.0)
...
def zero_command_base_planar_velocity_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    command_deadband: float = 0.025,
    velocity_deadband: float = 0.012,
    velocity_scale: float = 0.06,
    max_penalty: float | None = 2.0,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Penalize base x/y drift when the commanded velocity is zero."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband
    ...
    yaw_only_quat = yaw_quat(asset.data.root_quat_w)
    lin_vel_yaw = quat_apply_inverse(yaw_only_quat, asset.data.root_lin_vel_w)
    excess = torch.clamp(torch.abs(lin_vel_yaw[:, :2]) - velocity_deadband, min=0.0)
    penalty = torch.sum(torch.square(excess / max(velocity_scale, 1.0e-6)), dim=1)
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * cmd_small.to(penalty.dtype)


def zero_command_joint_motion_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg,
    command_deadband: float = 0.025,
    joint_velocity_deadband: float = 0.12,
    max_penalty: float | None = 2.0,
) -> torch.Tensor:
    """Penalize leg joint motion when the commanded velocity is zero."""
    command = _get_command_vector(env, command_name)
    cmd_small = torch.linalg.norm(command, dim=1) < command_deadband
    ...
    excess = torch.clamp(torch.abs(joint_vel) - joint_velocity_deadband, min=0.0)
    penalty = torch.mean(torch.square(excess), dim=1)
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * cmd_small.to(penalty.dtype)
...
def motion_hold_joint_position_error_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    hold_start_step: int,
    std: float,
    ...
    """Reward matching the reference joint pose only during the reference hold phase."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    joint_ids = _get_joint_ids(asset_cfg)
    hold = command.time_steps >= hold_start_step
    error = torch.square(command.joint_pos[:, joint_ids] - command.robot_joint_pos[:, joint_ids])
    reward = torch.exp(-error.mean(dim=-1) / std**2)
    return reward * hold.to(reward.dtype)


def motion_hold_joint_velocity_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    hold_start_step: int,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    joint_velocity_deadband: float = 0.08,
    max_penalty: float | None = 2.0,
) -> torch.Tensor:
    """Penalize residual joint motion only during the reference hold phase."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    joint_ids = _get_joint_ids(asset_cfg)
    hold = command.time_steps >= hold_start_step
    joint_vel = command.robot_joint_vel[:, joint_ids]
    excess = torch.clamp(torch.abs(joint_vel) - joint_velocity_deadband, min=0.0)
    penalty = torch.mean(torch.square(excess), dim=1)
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * hold.to(penalty.dtype)


def local_x_velocity_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    ...
    """Penalize forward/backward velocity whenever x command is inactive."""
    command = _get_command_vector(env, command_name)
    ...
    yaw_only_quat = yaw_quat(asset.data.root_quat_w)
    lin_vel_yaw = quat_apply_inverse(yaw_only_quat, asset.data.root_lin_vel_w)
    excess = torch.clamp(torch.abs(lin_vel_yaw[:, 0]) - velocity_deadband, min=0.0)
    penalty = torch.square(excess / max(velocity_scale, 1.0e-6))
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)

    return penalty * x_inactive.to(penalty.dtype)


def y_direction_local_x_velocity_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    direction: float,
    ...
    """Penalize local x drift only for one lateral command direction."""
    command = _get_command_vector(env, command_name)
    direction_sign = 1.0 if direction >= 0.0 else -1.0
    y_direction_active = command[:, 1] * direction_sign > command_threshold
    x_inactive = torch.abs(command[:, 0]) < x_command_deadzone
    ...
    return penalty * (y_direction_active & x_inactive).to(penalty.dtype)


def track_lateral_reference_velocity_with_heading_stability_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    command_threshold: float,
    ...
    """Track lateral velocity while discouraging yaw drift from the selected reference."""
    command_term: MotionCommand = env.command_manager.get_term(command_name)
    command = _get_command_vector(env, command_name)
    command_y = command[:, 1]
    ...
    reference_yaw = yaw_quat(command_term.anchor_quat_w)
    current_yaw = yaw_quat(command_term.robot_anchor_quat_w)
    yaw_error = quat_error_magnitude(reference_yaw, current_yaw)
    yaw_excess = torch.clamp(yaw_error - yaw_tolerance, min=0.0)
    reward *= torch.exp(-torch.square(yaw_excess) / yaw_std**2)

    active = torch.abs(command_y) >= command_threshold
    return reward * active.to(reward.dtype)


def y_command_local_x_velocity_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    command_threshold: float,
    x_command_deadzone: float,
    ...
    """Penalize forward/backward velocity while executing a lateral command."""
    command = _get_command_vector(env, command_name)
    y_active = torch.abs(command[:, 1]) > command_threshold
    x_inactive = torch.abs(command[:, 0]) < x_command_deadzone
    ...
    return penalty * (y_active & x_inactive).to(penalty.dtype)


def y_command_yaw_drift_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    command_threshold: float,
    yaw_command_deadzone: float,
    ...
    """Penalize heading drift during pure lateral commands."""
    command_term: MotionCommand = env.command_manager.get_term(command_name)
    command = _get_command_vector(env, command_name)
    y_active = torch.abs(command[:, 1]) > command_threshold
    yaw_inactive = torch.abs(command[:, 2]) < yaw_command_deadzone
    ...
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * (y_active & yaw_inactive).to(penalty.dtype)


def y_command_heading_deviation_reset_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    command_threshold: float,
    deadband: float,
    ...
    """Penalize lateral-motion heading drift from the episode reset heading."""
    command_term: MotionCommand = env.command_manager.get_term(command_name)
    command = _get_command_vector(env, command_name)
    command_y = command[:, 1]
    if direction is None:
        ...
        direction_sign = 1.0 if direction >= 0.0 else -1.0
        active = command_y * direction_sign > command_threshold

    asset = env.scene[asset_cfg.name]
    current_heading = yaw_quat(asset.data.root_quat_w)
    target_attr = f"_lateral_reset_heading_{asset_cfg.name}_{cache_key}"
    target_heading = getattr(env, target_attr, None)
    if target_heading is None or target_heading.shape != current_heading.shape:
        target_heading = current_heading.detach().clone()

    refresh = command_term.time_steps <= reset_steps
    if torch.any(refresh):
        target_heading = target_heading.clone()
        target_heading[refresh] = current_heading.detach()[refresh]
    setattr(env, target_attr, target_heading.detach())

    yaw_error = quat_error_magnitude(target_heading, current_heading)
    excess = torch.clamp(yaw_error - deadband, min=0.0)
    penalty = torch.square(excess / max(angle_scale, 1.0e-6))
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty * active.to(penalty.dtype)


def motion_tilt_error_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    deadband: float,
    max_penalty: float | None = 1.0,
) -> torch.Tensor:
    """Penalize tilt deviation from the selected reference orientation."""
    tilt_error, _ = _fixed_motion_tilt_error(env, command_name)
    penalty = torch.square(torch.clamp(tilt_error - deadband, min=0.0))
    if max_penalty is not None:
        penalty = torch.clamp(penalty, max=max_penalty)
    return penalty
...
def ready_gated_base_lin_vel_z_l2(
    env: ManagerBasedRLEnv,
    motion_command_name: str = "motion",
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Penalize vertical base velocity after ready without suppressing foot swing."""
    asset = env.scene[asset_cfg.name]
...
def ready_gated_base_ang_vel_xy_l2(
    env: ManagerBasedRLEnv,
    deadband: float = 0.0,
    motion_command_name: str = "motion",
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Penalize excessive roll/pitch angular velocity after ready without suppressing yaw."""
    asset = env.scene[asset_cfg.name]
    ang_vel_xy = torch.linalg.norm(asset.data.root_ang_vel_b[:, :2], dim=1)
    penalty = torch.square(torch.clamp(ang_vel_xy - deadband, min=0.0))
    motion_cmd: MotionCommand = env.command_manager.get_term(motion_command_name)
    return penalty * motion_cmd.ready.to(penalty.dtype)
...
def ready_gated_motion_relative_body_position_deviation_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    deadband: float,
    body_names: list[str] | None = None,
    ...
    """Penalize relative body-position deviation without blocking global x/y motion."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    error = torch.norm(
...
def ready_gated_motion_relative_body_orientation_deviation_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    deadband: float,
    body_names: list[str] | None = None,
    ...
    """Penalize relative body-orientation deviation after ready.

    The relative target follows the robot's current global yaw, so this preserves
    the hitch posture without opposing a commanded yaw rotation.
    """
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
...
def ready_gated_heading_yaw_drift_l2_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    yaw_command_deadzone: float,
    deadband: float,
    ...
    """Preserve the latest commanded heading whenever yaw rotation is inactive."""
...
def ready_gated_roll_error_l2_penalty(
    ...
    """Penalize yaw-invariant tilt deviation from the fixed final motion pose."""
...
def y_command_airborne_foot_clearance_reward(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    command_threshold: float,
    ...
    cache_attr = f"_y_command_airborne_clearance_indices_{sensor_cfg.name}"
    indices = getattr(env, cache_attr, None)
    if indices is None:
        sensor_body_ids = torch.as_tensor(sensor_cfg.body_ids, device=env.device, dtype=torch.long)
        sensor_indices = []
