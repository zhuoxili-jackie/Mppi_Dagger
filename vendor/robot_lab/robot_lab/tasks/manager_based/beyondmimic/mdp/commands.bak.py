...
class MotionCommand(CommandTerm):
    ...
    def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRLEnv):
        ...
        # stage: 0=track trajectory, 1=hold last frame, 2=velocity command tracking
    ...
    def _update_command(self):
        if not self.cfg.enable_stage_command:
            self.time_steps += 1
        ...
        if self.cfg.enable_stage_command:
            ...
            if hold_env_ids.numel() > 0:
                ...
                err_joint_pos = torch.norm(self.joint_pos[hold_env_ids] - self.robot_joint_pos[hold_env_ids], dim=-1)
                ...
                stable_now = (
                    ...
                    & (err_joint_pos < self.cfg.hold_joint_pos_threshold)
                ...
                promote = stable_reached & (self.post_stable_counter[hold_env_ids] >= self.cfg.extra_hold_steps_after_stable)
...
class MotionCommandCfg(CommandTermCfg):
    ...
    # Stage gating for stand-command:
    # 0(track trajectory) -> 1(hold last frame) -> 2(enable velocity command tracking)
    enable_stage_command: bool = False
    hold_anchor_rot_threshold: float = 0.12
    hold_body_pos_threshold: float = 0.06
    hold_joint_pos_threshold: float = 0.12
