...
### 2026-05-27_20-04-01/model_29999.pt 真机 0.2/0.3都能响应，左边不贴/右边贴，表现比较好
@configclass
...
class Stage2CommandEnvCfgV42(Stage2CommandEnvCfgV41):
    """V4.2: world-x correction + all-wheel vx/yaw pattern.

    Goal:
    - keep V41 delayed-actuator stability
    - keep y lateral walking along the platform
    - make vx command mean world-x motion near/far from the platform
    - make vx/yaw use all wheels, not only front-leg reaching
    - keep yaw as small world-z turning correction
    - avoid previous hard rear-wheel/front-rear-balance terms that damaged y walking
    """
    ...
    def __post_init__(self):
        ...
        # ------------------------------------------------------------
        # 1. Stage timing: keep V41/V32Safe-like stable timing.
        # ------------------------------------------------------------
        self.commands.motion.hold_stable_steps = 75
        self.commands.motion.extra_hold_steps_after_stable = 110
        self.commands.motion.max_hold_steps_before_force_command = 1600
        ...
        # 2. Command range.
        # ------------------------------------------------------------
        # vx: world-x near/far correction.
        # vy: main movement along platform.
        # yaw: small world-z turning correction.
        self.commands.base_velocity.ranges = mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.20, 0.20),
            lin_vel_y=(-0.30, 0.30),
            ang_vel_z=(-0.22, 0.22),
        ...
        # ------------------------------------------------------------
        # 3. Keep y as the main task.
        # ------------------------------------------------------------
        # y behavior is already correct, so do not convert it to world-frame.
        self.rewards.track_lin_vel_y.weight = 0.90
        self.rewards.track_lin_vel_y.params["std"] = 0.16
        self.rewards.track_lin_vel_y.func = mdp.stage_gated_track_lin_vel_axis_projected_body_exp
        self.rewards.track_lin_vel_y.params["axis"] = 1
        ...
        if not hasattr(self.rewards, "track_lin_vel_y_fine") or self.rewards.track_lin_vel_y_fine is None:
            ...
            self.rewards.track_lin_vel_y_fine.func = mdp.stage_gated_track_lin_vel_axis_projected_body_exp
            self.rewards.track_lin_vel_y_fine.weight = 0.10
            self.rewards.track_lin_vel_y_fine.params["axis"] = 1
            self.rewards.track_lin_vel_y_fine.params["std"] = 0.08
            self.rewards.track_lin_vel_y_fine.params["command_name"] = "base_velocity"
            self.rewards.track_lin_vel_y_fine.params["stage_command_name"] = "motion"
            self.rewards.track_lin_vel_y_fine.params["enabled_stage"] = 2

        # ------------------------------------------------------------
        # 4. x uses world frame.
        # ------------------------------------------------------------
        # This is the red-arrow direction in the viewer/world frame.
        self.rewards.track_lin_vel_xy = RewTerm(
            func=mdp.stage_gated_track_lin_vel_axis_world_exp,
            weight=0.20,
            params={
                "command_name": "base_velocity",
                "axis": 0,
        ...
        # ------------------------------------------------------------
        # 5. yaw uses world z angular velocity.
        # ------------------------------------------------------------
        # Do not overdrive yaw, otherwise the policy twists the body.
        self.rewards.track_yaw_rate = RewTerm(
            func=mdp.stage_gated_track_ang_vel_z_projected_exp,
            weight=0.13,
            params={
        ...
        # ------------------------------------------------------------
        # 6. New: all-wheel vx/yaw target pattern.
        # ------------------------------------------------------------
        # This replaces hard rear-wheel motion / front-rear balance.
        # It directly tells all four wheels what pattern they should follow.
        self.rewards.command_wheel_velocity_pattern = RewTerm(
            func=mdp.stage_gated_command_wheel_velocity_pattern_reward,
            weight=0.12,
            params={
                "command_name": "base_velocity",
                "asset_cfg": SceneEntityCfg("robot"),
        ...
        # Keep old separate wheel rewards weak or disable them to avoid conflicting targets.
        if hasattr(self.rewards, "x_cmd_wheel_vel"):
            self.rewards.x_cmd_wheel_vel = None
        if hasattr(self.rewards, "yaw_cmd_wheel_diff"):
            self.rewards.yaw_cmd_wheel_diff = None

        # ------------------------------------------------------------
        # 7. Very soft front-leg over-extension prevention.
        # ------------------------------------------------------------
        # Only prevents the obvious "front legs reach too far" solution.
        # Much weaker than the previous failed version.
        ...
        # ------------------------------------------------------------
        # 8. Keep front wheels on platform x margin, weakly.
        # ------------------------------------------------------------
        # Active only when no vx correction command is given.
        self.rewards.front_wheel_platform_x_margin = RewTerm(
            func=mdp.stage_gated_front_wheel_platform_x_margin_penalty,
            weight=-0.035,
            params={
                "asset_cfg": SceneEntityCfg(
                    "robot",
        ...
        # ------------------------------------------------------------
        # 9. Weak local-x drift helper.
        # ------------------------------------------------------------
        # This helps avoid body getting too close/far during pure y walking.
        # It turns off when user gives vx correction.
        if hasattr(self.rewards, "stage2_anchor_local_x_drift"):
            self.rewards.stage2_anchor_local_x_drift.weight = -0.045
            self.rewards.stage2_anchor_local_x_drift.params["deadband"] = 0.10
            self.rewards.stage2_anchor_local_x_drift.params["command_deadzone"] = 0.08
        ...
        # ------------------------------------------------------------
        # 12. Stage2 imitation balance.
        # ------------------------------------------------------------
        # Keep stage2 posture enough to avoid bad platform pose.
        self.rewards.motion_global_anchor_ori.params["stage2_weight"] = 0.08
        self.rewards.motion_body_pos.params["stage2_weight"] = 0.78
        self.rewards.motion_body_ori.params["stage2_weight"] = 0.72
        self.rewards.motion_joint_pos.params["stage2_weight"] = 0.62
        self.rewards.motion_joint_vel.params["stage2_weight"] = 0.30

        # ------------------------------------------------------------
        # 13. Disable failed hard terms.
        # ------------------------------------------------------------
        for name in [
            "rear_wheel_motion",
            "wheel_front_rear_balance",
            "stage2_base_box_x_clearance_penalty",
            "stage2_body_box_x_clearance_penalty",
            "stage2_base_lateral_edge_penalty",
            "stage2_feet_lateral_edge_penalty",
        ]:
            if hasattr(self.rewards, name):
                setattr(self.rewards, name, None)

        # ------------------------------------------------------------
        # 14. Preserve stage0/stage1 imitation protection.
        # ------------------------------------------------------------
        self.rewards.motion_global_anchor_pos.params["stage0_weight"] = 1.08
        self.rewards.motion_global_anchor_pos.params["stage1_weight"] = 0.90
        self.rewards.motion_global_anchor_pos.params["stage2_weight"] = 0.00
