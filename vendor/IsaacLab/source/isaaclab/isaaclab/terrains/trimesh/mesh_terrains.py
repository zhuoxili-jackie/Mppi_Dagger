...
def repeated_objects_terrain(
    ...
    return meshes_list, origin
def custom_mesh_terrain_fn(difficulty: float, cfg: mesh_terrains_cfg.CustomMeshTerrainCfg):
    mesh = trimesh.load(cfg.mesh_path, process=False)
    mesh = mesh.copy()

    bbox = mesh.bounds
    length_x = bbox[1][0] - bbox[0][0]
    length_y = bbox[1][1] - bbox[0][1]

    mesh.apply_translation([-bbox[0][0], -bbox[0][1], 0.0])

    z_min, z_max = cfg.height_range
    z_offset = (1.0 - difficulty) * z_min + difficulty * z_max

    x_mid = cfg.size[0] / 2
    y_offset = cfg.size[1] / 2 - (bbox[1][1] - bbox[0][1]) / 2
    mesh.apply_translation([2*cfg.size[0]/3, y_offset, z_offset])
    ...
    origin = np.array([cfg.size[0] * 0.5, cfg.size[1] * 0.5, 0], dtype=float)
    return mesh, origin